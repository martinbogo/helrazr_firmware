...
static void drawTable() {
    ...
    int maxRows = 5;
    int start = 0;
    if (curChan >= maxRows) start = curChan - maxRows + 1;
    if (start + maxRows > MESH_CHANNEL_COUNT) start = MESH_CHANNEL_COUNT - maxRows;
    if (start < 0) start = 0;
#endif
    for (int i = start; i < start + maxRows && i < MESH_CHANNEL_COUNT; i++) {
        int row = i - start;
#if HAS_OLED
        int y = 22 + row * 8;
#else
...
#else
            snprintf(rssiStr, sizeof(rssiStr), "%d", (int)stats[i].lastRSSI);
#endif
            uint32_t age = (now - stats[i].lastSeenMs) / 1000;
            if (age < 60) snprintf(ageStr, sizeof(ageStr), "%lus", age);
            else          snprintf(ageStr, sizeof(ageStr), "%lum", age / 60);
        }

#if HAS_OLED
        char rowStr[40];
        snprintf(rowStr, sizeof(rowStr), "%-9s %3d %s", MESH_CHANNELS[i].name, stats[i].packets, rssiStr);
        display_draw_text_small_abs(0, y, col, rowStr);
        ...
        display_line_text(110, stats[i].packets ? rssiColor(stats[i].lastRSSI) : (uint16_t)DISPLAY_CYAN, rssiStr);
        display_line_text(175, DISPLAY_CYAN, ageStr);
        display_end_line();
#endif
    }
#if !HAS_OLED
    for (int row = MESH_CHANNEL_COUNT - start; row < maxRows; row++) {
        display_begin_line(36 + row * 19, false);
        display_end_line();
    }
#endif
    ...
    display_draw_text_small_line(0, 126, DISPLAY_YELLOW, foot);
#endif
}
